...
const FaceCapture: React.FC<FaceCaptureProps> = ({ onCapture, onSkip }) => {
  // Refs
  ...
  const [facePositionStatus, setFacePositionStatus] = useState<FacePositionStatus>('none');
  ...
  const checkFacePosition = useCallback((face: Face): boolean => {
    ...
    const guideSizeRatio = (guideWidth * guideHeight) / (canvasWidth * canvasHeight);
  ...
  const updateFacePositionStatus = useCallback((faces: Face[]) => {
    if (faces.length === 0) {
  ...
  }, [setupCamera, runFaceDetection, isVideoReady]);
  ...
  const handleTryAgain = useCallback(async () => {
    ...
    // Restart camera and face detection
    try {
      await setupCamera();
      await runFaceDetection();
    } catch (error) {
  ...
  const StatusIndicator = () => {
    const statusConfig = {
      ...
      detected: { icon: '⚠', text: 'Face detected - fit within the guide', className: 'detected-but-positioned' },
      positioned: { icon: '✓', text: 'Face positioned correctly', className: 'positioned' }
    };
    ...
    return (
      <div className="face-capture-container">
        ...
        <div className="capture-controls">
          <button className="capture-button ready" onClick={handleNext}>
            ✓ Next
          </button>
          <button className="skip-button" onClick={handleTryAgain}>
            Try Again
          </button>
        </div>
      </div>
    ...
    <div className="face-capture-container">
      ...
      {isLoading ? (
        <div className="loading-spinner">
          <div className="shampoo-spinner">
            <img src="/assets/aqua.png" alt="Gliss Aqua Revive" className="spinning-bottle" />
        ...
        <div className="capture-controls">
          {facePositionStatus === 'positioned' ? (
            <button className="capture-button ready" onClick={handleCapture}>
              📸 Capture Photo
            </button>
          ) : (
            <div className="capture-button disabled">
              📸 {facePositionStatus === 'none' ? 'Face not detected' : 'Fit your face within the guide'}
            </div>
          )}
